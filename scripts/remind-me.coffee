...
#
# Commands:
#   @bot remind me in <time> to <action> - Set a reminder. <time> is in the format 1 day, 2 hours, 5 minutes.
#
# Author:
#   whitman
#
# Retrieved on 2013-05-13 from https://github.com/github/hubot-scripts/blob/master/src/scripts/remind.coffee
    ...
    if @cache.length > 0
      ...
      if @cache.length > 0
        trigger = =>
          ...
          @robot.send reminder.for, '@' + reminder.for.mention_name + ' reminder to ' + reminder.action
          @queue()
